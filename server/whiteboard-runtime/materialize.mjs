...
export function materialize({ updates, title }) {
  ...
  try {
    ...
    // Asset integration is deferred; preserve document records without inspecting them.
    return { state: Buffer.from(Y.encodeStateAsUpdate(doc)).toString('base64'), title };
  } finally { doc.destroy(); }
}
