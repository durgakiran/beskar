...
func (c Content) createDocumentNode() Document {
...
func recursiveContentTree(id uuid.UUID, idToContentMap map[uuid.UUID]Content, idToChildMap map[uuid.UUID][]uuid.UUID) Document {
	if content, ok := idToContentMap[id]; ok {
		childIds := idToChildMap[id]
		doc := content.createDocumentNode()
		...
		for i, childId := range childIds {
			doc := recursiveContentTree(childId, idToContentMap, idToChildMap)
			childDocumentList[i] = doc
		}
		doc.Content = childDocumentList
		return doc
	} else {
...
func (i Doc) CreateEditorDocument() Document {
	data := i.Data
	idToContentMap := make(map[uuid.UUID]Content)
	...
	doc := recursiveContentTree(rootId, idToContentMap, idToChildIdMap)
