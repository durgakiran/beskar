...
Notification work is split into focused requirements so the backend can build the reusable delivery engine first, then expand product triggers and in-app notification flows deliberately.
...
| Notification engine and flow expansion | `req-notification-engine-and-flow-expansion.md` | Expand invite-only notifications into a general in-app notification feed and define email requirements for all eligible flows. |

## Recommended Build Order

1. Build the email notification engine.
2. Integrate one V1 trigger, preferably space invite email.
3. Add the generic in-app notification feed and service layer.
4. Add more triggers only when the product flow is implemented and the notification has clear user value.
...
Product trigger selection is handled in `req-trigger-integration.md`. The broader in-app and multi-flow expansion is handled in `req-notification-engine-and-flow-expansion.md`.
