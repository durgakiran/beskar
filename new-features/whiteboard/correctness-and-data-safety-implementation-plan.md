...
### 20.6 Implementation status — package and Beskar host path complete; operational rollout pending

- Glideline `0.0.9` adds a strict untrusted-content boundary. SVG uploads are byte-, element-, nesting-, path-command-, and coordinate-limited; DTDs/entities, scripts, handlers, foreign content, external resources, CSS URLs, transforms, filters, animation, unsupported namespaces/elements, and unsafe paint values are rejected. Accepted input becomes canonical path/viewBox data with a SHA-256 content identity; raw XML is never persisted.
- `sanitized-svg` records are rendered only by constructing engine-owned SVG elements. The schema revalidates canonical asset records on every store/load/import boundary and rejects arbitrary `src`, URL, HTML, raw SVG, data, or byte fields in document assets. Developer-installed `createSvgPathShape` callbacks remain trusted startup code and are not exposed as an upload format.
- Clipboard HTML is currently normalized to bounded plain text because Glideboard has no structured rich-text model. Active/embed elements and resource loads are removed; raw browser HTML is never stored. This intentionally does not claim rich-text formatting support.
- PNG, JPEG, and WebP bytes are signature-sniffed, hashed, MIME-checked, and bounded by encoded bytes, dimensions, and decoded pixels. Glideboard `0.0.11` requires a host `assetStorage` adapter: validated bytes must persist successfully before an immutable asset/shape transaction is inserted, while runtime media URLs come from the trusted resolver and are never written into the board document. In-flight imports are aborted on session disposal.
- Glideboard paste routes external SVG, raster files, HTML, and plain text through these ingress paths. Its imperative handle also exposes safe SVG/raster import. Internal copy/paste remains the graph-aware Glideline clipboard format.
- The malicious corpus covers scripts, handlers, `foreignObject`, remote/data resources, DTDs/entities, transforms, extreme coordinates, unsafe stored URLs, malformed canonical records, active HTML, MIME mismatch, raster pixel bombs, engine-owned rendering, immutable identity, persistence-before-reference, and failed-storage rollback.
- Beskar now supplies Glideboard's `assetStorage` adapter. It uploads raster bytes to a page-scoped content-hash endpoint before the Yjs/store record can reference them and resolves that same immutable authenticated URL for editable and historical boards.
- The server independently re-sniffs PNG/JPEG/WebP bytes, recomputes SHA-256, rejects MIME/hash mismatches, enforces encoded-byte/dimension/pixel limits, and has no URL-fetch input. A page-and-hash catalog retains immutable objects for the lifetime of the whiteboard, quota reconciliation includes them, and page deletion removes their objects only after the database transaction commits.
- Media delivery checks page `view` permission before page-scoped lookup and sends exact raster MIME with `nosniff`, sandbox CSP, same-origin CORP, no-referrer, and private immutable caching. Tests cover hostile/active formats, URL-shaped input, MIME/hash mismatch, decompression-bomb limits, cross-page permission denial, delivery headers, and the UI upload/resolver contract.

The code path is implemented, but broad external-upload rollout still requires deployment evidence: apply and verify the Liquibase migration, exercise the configured object store and quota modes, run the browser CSP/network corpus in the deployed origin topology, and complete the security/operations review listed in 20.5. The current server inspection service is a strict in-process data boundary; deployments requiring process-level decoder isolation must move that interface behind a sandboxed worker without changing the document or Glideboard contracts.
