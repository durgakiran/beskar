...
export default function Page() {
    ...
    const validUnknownEmails = useMemo(() => (searchData?.data?.unknownEmails ?? []).filter((candidate) => EMAIL_REGEX.test(candidate.email)), [searchData]);
    ...
    const toggleMatch = (candidate: MemberCandidate) => {
        setSelectedMatches((current) => (current.some((item) => item.email === candidate.email) ? current.filter((item) => item.email !== candidate.email) : [...current, candidate]));
    };

    const toggleUnknown = (candidate: MemberCandidate) => {
        setSelectedUnknowns((current) => (current.some((item) => item.email === candidate.email) ? current.filter((item) => item.email !== candidate.email) : [...current, candidate]));
    };
    ...
    const submit = async () => {
        ...
        try {
            for (const candidate of [...selectedMatches, ...selectedUnknowns]) {
                ...
                if (response.ok) {
                    ...
                    failures.push(mapQuotaErrorMessage(body?.error?.detail || body?.error?.message || `Unable to invite ${candidate.email}`));
                }
            }
        } finally {
            setSubmitting(false);
        ...
        if (failures.length > 0) {
            setToast({
                type: "warning",
                message: invitedCount > 0 ? `Completed with issues: ${invitedCount} invites sent, ${failures.length} failed. ${failures[0]}` : failures[0],
            });
            return;
        }
    ...
    return (
        <div className="space-y-6">
            <SettingsBreadcrumb items={[{ label: "Settings", href: `/space/${spaceId}/settings/users` }, { label: "Users", href: `/space/${spaceId}/settings/users` }, { label: "Invite User" }]} />

            <button type="button" onClick={navigateBack} className="inline-flex w-fit items-center gap-2 text-sm font-medium text-neutral-500 transition-colors hover:text-primary-700">
                <FiChevronLeft className="h-4 w-4" />
                <span>Users</span>
            </button>

            <SettingsPageHeader title="Invite User" subtitle="Search for an existing member or paste one or more emails to invite people into this space." />
            ...
            <div className="space-y-5 rounded-xl border border-neutral-200 bg-white p-5">
                <div className="space-y-2">
                    ...
                    {invalidEmails.length > 0 ? <div className="text-sm text-amber-700">Invalid emails will be ignored: {invalidEmails.join(", ")}</div> : null}
                </div>
            ...
            {toast ? <ToastComponent icon={toast.type === "success" ? "Check" : "AlertTriangle"} message={toast.message} toggle type={toast.type} /> : null}
        </div>
    );
}
