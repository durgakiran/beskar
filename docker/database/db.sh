...
# bring up databases
docker compose -f $WD/db.yml --env-file $WD/.env -p beskar-db up -d
...
docker run -d --name $CONTAINER -v //$DIRECTORY:/app --network custom_local_network --env-file $WD/.env $IMAGE  -it
